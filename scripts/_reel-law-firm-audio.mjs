...
import { LINES, VOICE, ensureVoiceoverLines } from "./_reel-law-firm-vo-lines.mjs";

// Scene schedule — must match scripts/_reel-law-firm.html's SCENE_DEFS
...
const sceneStarts = LINES.map((_, i) => {
  const start = cursor;
  cursor = start + [7.82, 4.54, 7.03, 6.58, 5.18, 8.18, 6.89][i] + GAP;
  return start;
});
...
voClips.forEach((c, i) => {
  console.log(` vo${i}: ${c.duration.toFixed(2)}s (expected ${[7.82, 4.54, 7.03, 6.58, 5.18, 8.18, 6.89][i]}s) - ${c.text}`);
...
const videoIn = "D:/zenith-studio/insta/reels/law-firm-ai-team-silent.mp4";
const tmpOut = "D:/zenith-studio/insta/reels/law-firm-ai-team.mp4";
