...
import { Icon } from "../../_halo/Icon";
import { Eyebrow, StatusChip } from "../../_halo/ui";
import { agents, type StepState } from "../../sample-data";
import shell from "../../_halo/shell.module.css";
import tokens from "../../_halo/tokens.module.css";
import u from "../../_halo/ui.module.css";
...
export default async function AgentDetailPage({ params }: { params: Promise<{ agentId: string }> }) {
  ...
  const { steps, run, details, recentRuns } = agent;
    ...
    <main className={`${shell.main} ${s.main}`}>
      ...
      <header className={s.header}>
        <nav aria-label="Breadcrumb" className={s.breadcrumb}>
          <Link href="/lab/dashboard/preview">Agents</Link>
          ...
          <div className={s.identity}>
            ...
            <div className={s.identityText}>
              <div className={s.titleRow}>
                ...
                <StatusChip tone={agent.statusTone} height={26}>
                  {agent.statusLabel}
                </StatusChip>
              </div>
              <span className={s.description}>{agent.description}</span>
            </div>
          </div>
      ...
      <div className={s.body}>
        <section className={`${tokens.dots} ${s.workflow}`} aria-label="Workflow, current run">
        ...
        <div className={s.side}>
          ...
          <section className={`${u.listCard} ${s.card}`} aria-label="Details">
            ...
            {agent.detailsBadge && (
              <div className={`${s.row} ${s.rowCenter}`}>
                <span className={s.rowKey}>{agent.detailsBadge.label}</span>
                <StatusChip tone={agent.statusTone === "live" ? "done" : "attention"} height={24} padX={10}>
                  {agent.detailsBadge.value}
                </StatusChip>
              </div>
            )}
          </section>
