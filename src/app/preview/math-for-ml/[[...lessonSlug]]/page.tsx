...
const LESSON_COMPONENTS: Record<number, Record<string, React.ComponentType<any>>> = {
  ...
  60: {
    Objectives,
    Callout,
    EmbeddingDetectiveSection: MathDetectiveSections.EmbeddingDetectiveSection,
    PcaDetectiveSection: MathDetectiveSections.PcaDetectiveSection,
  ...
  4: { Objectives, Callout, GradientLabSection: Module04.GradientLabSection, MathLevelsSection: Module04.MathLevelsSection, QuizSection: Module04.QuizSection },
  5: { Objectives, Callout, GradientDescentLabSection: Module05.GradientDescentLabSection, MathLevelsSection: Module05.MathLevelsSection, QuizSection: Module05.QuizSection },
  6: { Objectives, Callout, ProbabilityLabSection: Module06.ProbabilityLabSection, MathLevelsSection: Module06.MathLevelsSection, QuizSection: Module06.QuizSection },
  7: { Objectives, Callout, SamplingLabSection: Module07.SamplingLabSection, MathLevelsSection: Module07.MathLevelsSection, QuizSection: Module07.QuizSection },
  8: { Objectives, Callout, LikelihoodLabSection: Module08.LikelihoodLabSection, MathLevelsSection: Module08.MathLevelsSection, QuizSection: Module08.QuizSection },
  9: { Objectives, Callout, EntropyLabSection: Module09.EntropyLabSection, MathLevelsSection: Module09.MathLevelsSection, QuizSection: Module09.QuizSection },
  10: { Objectives, Callout, NeuronLabSection: Module10.NeuronLabSection, MathLevelsSection: Module10.MathLevelsSection, QuizSection: Module10.QuizSection },
  11: { Objectives, Callout, AttentionLabSection: Module11.AttentionLabSection, MathLevelsSection: Module11.MathLevelsSection, QuizSection: Module11.QuizSection },
  ...
  205: { Objectives, Callout, WorkspaceSection: Project06.WorkspaceSection, MathSection: Project06.MathSection, BriefSection: Project06.BriefSection },
};
...
export default async function PreviewLessonPage({
  ...
  if (!slug) {
    redirect(`/preview/${COURSE_ID}/${firstLesson().slug}`);
  }
  ...
  const body = (
    <div className="mx-auto max-w-[820px] px-6 pb-24">
      ...
      <footer className="flex items-center justify-between border-t border-[#232838] py-8">
        {prev ? (
          <a href={`/preview/${COURSE_ID}/${prev.slug}`} className="text-[13px] text-[#9aa0ae] hover:text-[#8b7cf6]">
            ← {prev.title}
          </a>
        ) : (
          <span />
        ...
        {next ? (
          <a href={`/preview/${COURSE_ID}/${next.slug}`} className="text-[13px] font-semibold text-[#8b7cf6]">
            {next.title} →
          </a>
        ) : (
  ...
  return (
    <LearnShell data={railData} courseId={COURSE_ID} activeModuleId={lesson.id} basePath={`/preview/${COURSE_ID}`} courseLabel={courseLabel}>
      {body}
    </LearnShell>
  );
}
