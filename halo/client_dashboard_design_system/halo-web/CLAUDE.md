...
| `/showcase`      | AI agent files animation (demo route) | canvas board "Animation · AI agent files" |
...
## AI agent files animation (`components/AgentFiles/`)
- Use anywhere: `import { AgentFiles } from "@/components/AgentFiles";` then `<AgentFiles holdSeconds={5.5} autoplay />`. It's a client component.
- Copy lives in `agents.ts`. Keep 3 agents or update `SLOTS` / `AGENT_SLOT` in `AgentFiles.tsx` (agents sit in folder slots 1, 3, 5 of 7).
- The stage is authored at a fixed 1280x800 and scaled to its container's width (ResizeObserver), so every position and transform in the file is in 1280x800 design pixels. Don't convert them to responsive units.
- Timeline per agent: 450ms folder slides out, 1300ms card + staggered text reveal, holds `holdSeconds`, 1000ms return, then the next agent. The pills jump to an agent and restart the timer.
- It pauses when scrolled off screen (IntersectionObserver) and drops transitions under `prefers-reduced-motion`.
- Below ~700px wide the text gets small because the whole stage scales. For phones, build a stacked variant rather than shrinking this one.
